...
class Encounter():
    ...
    def run(self, update_progress):
        ...
        while True:
            ...
            if C.log_sim >= 0:
                print('CHECK2 {:7.1f} {}'.format(self._arrays['global']['total_damage'][C.log_sim], self._damage[C.log_sim]))
        ...
        if not over_time: # no summary for comparison mode
            if C.log_sim >= 0:
                print('total non-ignite damage {:7.1f} {} {}'.format(self._arrays['global']['total_damage'][C.log_sim], self._arrays['player']['cast_number'].shape[1], self._arrays['global']['duration'][C.log_sim]))
