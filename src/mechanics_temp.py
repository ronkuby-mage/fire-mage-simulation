...
def subtime(arrays, sub, add_time):
    arrays['global']['running_time'][sub] += add_time
    arrays['player']['cast_timer'][sub, :] -= add_time
    arrays['player']['spell_timer'][sub, :] -= add_time
...
def do_cast(arrays, still_going, cast_array):
    cast_ends = np.where(cast_array)[0]
    add_time = -1.0
    if cast_ends.size > 0:
        cst = still_going[cast_ends]
        next_hit = np.argmin(arrays['player']['cast_timer'][cst, :], axis=1)
        add_time = np.min(arrays['player']['cast_timer'][cst, :], axis=1, keepdims=True)
        subtime(arrays, cst, add_time)
        ...
        # do global cooldown transfer to cast here
...
def advance(arrays, plus_damage, hit_chance, crit_chance, rotation, sim_size):
    ...
    going_array &= np.logical_not(arrays['global']['has_decision'])
    still_going = np.where(going_array)[0]
    if still_going.size == 0:
        return False

    # cast finished
    cast_timer = np.min(arrays['player']['cast_timer'][still_going], axis=1, keepdims=True)
    spell_timer = np.min(arrays['player']['spell_timer'][still_going], axis=1, keepdims=True)
    ignite_timer = arrays['boss']['ignite_timer'][still_going]
    tick_timer = arrays['boss']['tick_timer'][still_going]
    scorch_timer = arrays['boss']['scorch_timer'][still_going]

    ignite_count = arrays['boss']['ignite_count'][still_going]
    scorch_count = arrays['boss']['scorch_count'][still_going]
    cast_array = cast_timer < spell_timer
    cast_array &= np.logical_not(ignite_count) | ((cast_timer < ignite_timer) | (cast_timer < tick_timer))
    cast_array &= np.logical_not(scorch_count) | (cast_timer < scorch_timer)
    do_cast(arrays, still_going, cast_array)
    
    # only dealing with non-casts so no need to copy array previously
    going_array &= np.logical_not(cast_array)
    still_going = np.where(going_array)[0]
    ...
    ignite_count = arrays['boss']['ignite_count'][still_going]
    scorch_count = arrays['boss']['scorch_count'][still_going]
    spell_array = np.logical_not(ignite_count) | ((spell_timer < ignite_timer) | (spell_timer < tick_timer))
    spell_array &= np.logical_not(scorch_count) (spell_timer < scorch_timer)
    do_spell(arrays, still_going, spell_array)
