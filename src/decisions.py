...
class Decider():
    ...
    def _common(self, arrays, still_going, next_hit):
        ...
        action = self._rotation['initial']['common'][stage]
        ...
        ss_needed = C._SCORCHES[self._num_mages]
        ...
        # other actions
        regular = np.where(np.logical_not(scorch_stage))
        if regular.size:
            decisions[regular] = C._DECIDE[action]
    ...
    def _special(self, arrays, still_going, next_hit):
        ...
        if "have_pi" in self._rotation['initial']:
            ...
            if pi.size:
                action = self._rotation['initial']['have_pi'][stage[pi]]
                ...
                if cooldown.size:
                    ...
                    if buff.size:
                        ...
                        decision[pi][cooldown][buff] = cast
                ...
                if not_cooldown.size:
                    decisions[pi][not_cooldown] = C._DECIDE[action][not_cooldown]
        ...
        action = self._rotation['initial']['other'][stage[other]]
        frostbolt_stack = action == "frostbolt_stack"
        frostbolt = np.where(frostbolt_stack)[0]

        if frostbolt.size:
            mod_sg = still_going[other][frostbolt]
            mod_nh = next_hit[other][frostbolt]
            not_stack = array['boss']['ignite_count'][still_going[other][frostbolt]] < C._IGNITE_STACK
        ...
        regular = np.where(np.logical_not(frostbolt_stack))
        if regular.size:
            decisions[regular] = C._DECIDE[action[regular]]
            self._stage[still_going[other][regular], next_hit[other][regular]] += 1
        ...
        if "special" in self._rotation['continuing']:
            ...
            if special.size:
                if self._rotation['continuing']['special']['value'] == 'maintain_scorch':
                    need_scorch = (arrays['boss']['scorch_time'] < C._MAX_SCORCH_REMAIN) |\
                                  (arrays['boss']['scorch_count'] < C._SCORCH_STACK)
                    no_scorch = np.where(np.logical_not(need_scorch))[0]
                    scorch = np.where(need_scorch)[0]
                    if scorch.size:
                        decisions[special][scorch] = C._CAST_SCORCH
                else:
                    no_scorch = np.arange(special.size)
                decisions[special][no_scorch] =\
                    C._DECIDE[self._rotation['continuing']['special']['value']]
        else:
    ...
    def get_decisions(self, arrays, still_going):
        ...
        next_hit = np.argmin(array['player']['cast_timer'][still_going, :], axis=1)

        while True:
            no_decision = np.where(decision < 0)[0]
            ...
            pre_cont = self._rotation['initial']['common'] + self._rotation['initial']['other']
